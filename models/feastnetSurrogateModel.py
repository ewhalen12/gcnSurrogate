...
###############################################################################
    def predict(self, inputs):
        ...
        return preds
###############################################################################
    def predictNew(self, inputs):       
        # prep data
        input_mean = torch.tensor(self.ss.mean_.tolist(), dtype=torch.float64)
        input_std = torch.tensor(self.ss.scale_.tolist(), dtype=torch.float64)
        
        inputsScaled = self.applySS(inputs)
        newGraph = inputs[0].clone()
        newGraph.y = (newGraph.y -input_mean)/input_std
        inputsScaled = [newGraph]
        testLoader = tg.data.DataLoader(inputsScaled, batch_size=1, shuffle=False)
        ...
        for batch in testLoader:
            ...
            out = self(batch)
            #undo SS 
            p = (input_std*out.cpu())+input_mean 
            preds.append(p)
